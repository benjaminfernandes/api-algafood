...
public class PedidoConverter extends RepresentationModelAssemblerSupport<Pedido, PedidoModel>
		implements Converter<Pedido, PedidoModel, Pedidoinput> {
	...
	public PedidoModel toModel(Pedido domain) {
		...
		/*
		 * pedidoModel.getFormaPagamento().add(linkTo(methodOn(FormaPagamentoController.
		 * class) .buscar(domain.getFormaPagamento().getId(), null)).withSelfRel());
		 * 
		 ...
		 * 
		 * pedidoModel.getEndereco().getCidade().add(linkTo(methodOn(CidadeController.
		 * class) .buscar(domain.getEndereco().getCidade().getId())).withSelfRel());
		 * 
		...
		if (algaSecurity.podeConsultarRestaurantes()) {
			pedidoModel.getRestaurante().add(algalinks.linkToRestaurante(domain.getRestaurante().getId()));
		}

		if (algaSecurity.podeConsultarUsuariosGruposPermissoes()) {
			pedidoModel.getCliente().add(algalinks.linkToUsuario(domain.getCliente().getId()));
		}

		if (algaSecurity.podeConsultarFormasPagamento()) {
			pedidoModel.getFormaPagamento().add(algalinks.linkToFormaPagamento(domain.getFormaPagamento().getId()));
		}

		if (algaSecurity.podeConsultarCidades()) {
			pedidoModel.getEndereco().getCidade().add(algalinks.linkToCidade(domain.getEndereco().getCidade().getId()));
		}
